...
local function emit_changed()
    ...
    local global_settings = runpoon.get_global_settings()
...
local function filter_filetype()
    ...
    local excluded_filetypes = runpoon.get_global_settings().excluded_filetypes

    if current_filetype == "runpoon" then
        log.error("filter_filetype(): You can't add runpoon to the runpoon")
        error("You can't add runpoon to the runpoon")
        return
    end
...
function M.get_index_of(item, marks)
    ...
    if item == nil then
        ...
        error(
            "You have provided a nil value to runpoon, please provide a string rep of the file or the file idx."
        )
        return
    end
    ...
    if type(item) == "string" then
        ...
        if marks == nil then
            marks = runpoon.get_mark_config().marks
        end
    ...
    -- TODO move this to a "runpoon_" prefix or global config?
...
function M.add_file(file_name_or_buf_id)
    ...
    runpoon.get_mark_config().marks[found_idx] = create_mark(buf_name)
...
function M.remove_empty_tail(_emit_on_changed)
    ...
    local config = runpoon.get_mark_config()
    local found = false
...
function M.store_offset()
    ...
    local ok, res = pcall(function()
        local marks = runpoon.get_mark_config().marks
...
function M.rm_file(file_name_or_buf_id)
    ...
    runpoon.get_mark_config().marks[idx] = create_mark("")
    M.remove_empty_tail(false)
    emit_changed()
end

function M.clear_all()
    runpoon.get_mark_config().marks = {}
...
function M.get_marked_file(idxOrName)
    ...
    return runpoon.get_mark_config().marks[idxOrName]
end
...
function M.get_marked_file_name(idx, marks)
    ...
    if marks ~= nil then
        ...
        mark = runpoon.get_mark_config().marks[idx]
    end
