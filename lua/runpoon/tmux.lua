...
if global_config.tmux_autoclose_windows then
    local runpoon_tmux_group = vim.api.nvim_create_augroup(
        "runpoon_TMUX",
        { clear = true }
    )
    ...
    vim.api.nvim_create_autocmd("VimLeave", {
        callback = function()
            require("runpoon.tmux").clear_all()
        end,
        group = runpoon_tmux_group,
    })
end
...
function M.sendCommand(idx, cmd, ...)
    ...
    if type(cmd) == "number" then
        cmd = runpoon.get_term_config().cmds[cmd]
    end
...
function M.get_length()
    ...
    return table.maxn(runpoon.get_term_config().cmds)
end
...
function M.emit_changed()
    ...
    if runpoon.get_global_settings().save_on_change then
        runpoon.save()
    end
end
...
function M.add_cmd(cmd)
    ...
    runpoon.get_term_config().cmds[found_idx] = cmd
    M.emit_changed()
end
...
function M.rm_cmd(idx)
    ...
    table.remove(runpoon.get_term_config().cmds, idx)
    M.emit_changed()
end

function M.set_cmd_list(new_list)
    log.trace("set_cmd_list(): New list:", new_list)
    for k in pairs(runpoon.get_term_config().cmds) do
        runpoon.get_term_config().cmds[k] = nil
    end
    for k, v in pairs(new_list) do
        runpoon.get_term_config().cmds[k] = v
    end
    M.emit_changed()
end
