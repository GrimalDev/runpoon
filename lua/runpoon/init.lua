...
local user_config = string.format("%s/runpoon.json", config_path)
local cache_config = string.format("%s/runpoon.json", data_path)

local M = {}

local grimaldev_runpoon = vim.api.nvim_create_augroup("GRIMALDEV_RUNPOON", { clear = true })

vim.api.nvim_create_autocmd({ "BufLeave", "VimLeave" }, {
	callback = function()
		require("runpoon.mark").store_offset()
	end,
	group = grimaldev_runpoon,
})
...
vim.api.nvim_create_autocmd("FileType", {
	...
	callback = function()
		-- Open runpoon file choice in useful ways
		--
...
--]]
runpoonConfig = runpoonConfig or {}
...
local function ensure_correct_config(config)
	...
	if proj.term == nil then
		log.debug("ensure_correct_config(): No terminal commands found for", mark_key)
		proj.term = { cmds = {} }
	end
...
function M.save()
	...
	Path:new(cache_config):write(vim.fn.json_encode(runpoonConfig), "w")
end
...
function M.setup(config)
	...
	if complete_config.tabline then
		require("runpoon.tabline").setup(complete_config)
	end

	runpoonConfig = complete_config

	log.debug("setup(): Complete config", runpoonConfig)
	log.trace("setup(): log_key", Dev.get_log_key())
end

function M.get_global_settings()
	log.trace("get_global_settings()")
	return runpoonConfig.global_settings
end
...
function M.refresh_projects_b4update()
	...
	local current_p_config = {
		projects = {
			[cwd] = ensure_correct_config(runpoonConfig).projects[cwd],
		},
	}

	-- erase all projects from global config, will be loaded back from disk
	runpoonConfig.projects = nil
	...
	-- don't override non-project config in runpoonConfig later
	...
	local complete_config = merge_tables(runpoonConfig, expand_dir(c_config), expand_dir(current_p_config))
	...
	runpoonConfig = complete_config
	log.debug("refresh_projects_b4update(): Complete config", runpoonConfig)
	log.trace("refresh_projects_b4update(): log_key", Dev.get_log_key())
end

function M.get_term_config()
	log.trace("get_term_config()")
	return ensure_correct_config(runpoonConfig).projects[utils.project_key()].term
end

function M.get_mark_config()
	log.trace("get_mark_config()")
	return ensure_correct_config(runpoonConfig).projects[mark_config_key()].mark
end

function M.get_menu_config()
	log.trace("get_menu_config()")
	return runpoonConfig.menu or {}
end

-- should only be called for debug purposes
function M.print_config()
	print(vim.inspect(runpoonConfig))
end
